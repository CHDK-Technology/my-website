...
//
// Stage 2: catalogue listing + public preview + full Razorpay-gated
// purchase/download flow. This replaces the Stage 1 (listing-only) version.
// Requires RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET to be set in the
// environment (Render dashboard + local .env), and the `razorpay` package
// installed (see backend/package.json).
...
/* ======= PUBLIC PREVIEW (only if previewFileName is set) ====== */
...
router.post('/verify', paymentLimiter, async (req, res) => {
  try {
    ...
    const doc = await Document.findById(purchase.document);
